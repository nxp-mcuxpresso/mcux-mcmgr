...
def main():
    ...
    PRIMARY_GCOV_CONF = current_dir / "middleware" / "multicore" / "mcmgr" / "tests" / "prj-gcov-primary.conf"
    PRIMARY_GCOV_SINGLECORE_CONF = current_dir / "middleware" / "multicore" / "mcmgr" / "tests" / "prj-gcov-primary-singlecore.conf"
    SECONDARY_GCOV_CONF = current_dir / "middleware" / "multicore" / "mcmgr" / "tests" / "prj-gcov-secondary.conf"
    ...
    for test in TESTS:
        ...
        # Build the test path
        test_path = Path("middleware") / "multicore" / "mcmgr" / "tests" / test / "primary"

        # Get test name for secondary core config parameter
        test_name = test.replace("/", "_")

        # Select appropriate config file based on test type
        if test == "test_api_functions":
            primary_conf = PRIMARY_GCOV_SINGLECORE_CONF
        else:
            primary_conf = PRIMARY_GCOV_CONF
        ...
        cmd = [
            ...
            "--",
            f"-DCONF_FILE={str(primary_conf)}",
            f"-D{test_name}_secondary_core_CONF_FILE={str(SECONDARY_GCOV_CONF)}",
